...
public class PathingMain extends PApplet
{
    ...
    */
    ...
    private boolean canMove(Point pos, List<Point> DirectionList, int tracker, List<Point> path, int pindex)
    {
        Point newpos = new Point(pos.x + DirectionList.get(tracker).x, pos.y + DirectionList.get(tracker).y);
...
//          //check if my right neighbor is the goal
            ...
            pos = newpos;
            canMove(pos, DirectionList, 0, path, pindex + 1);
        }
        else
        {
            //Right,Left,Down,Up
            if(tracker + 1 <= 3)
                canMove(pos, DirectionList, tracker+1, path, pindex);
            else {
                pindex--;
                pos = path.get(pindex);
                path.remove(path.get(pindex));
                canMove(pos, DirectionList, 0, path, pindex);
            }
        }
        return false;
    }
    ...
    private boolean moveOnce(Point pos, GridValues[][] grid, List<Point> path)
    {
        ...
        Directions.addAll(Arrays.asList(RIGHT,LEFT,DOWN,UP));

        canMove(rightN, Directions, 0, path, 0);
